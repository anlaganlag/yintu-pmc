...
# -*- coding: utf-8 -*-
"""
基于ALL_PMC_ORDER.csv的全面订单欠料分析系统
594个订单的精准欠料分析 + ROI优先级排序
"""
...
class CSVOrderShortageAnalyzer:
    def __init__(self):
        self.order_csv_df = None         # CSV订单清单数据
        self.shortage_df = None          # 欠料数据
        self.inventory_df = None         # 库存价格数据
        self.supplier_df = None          # 供应商数据
        self.orders_df = None            # 原始订单数据（用于获取订单金额）
    ...
    def load_csv_order_list(self, csv_file='ALL_PMC_ORERDER.csv'):
        """加载CSV订单清单"""
        print("=== 🔄 加载CSV订单清单 ===")
        
        try:
            # 读取CSV文件，处理中文编码
            self.order_csv_df = pd.read_csv(csv_file, encoding='utf-8-sig')
            print(f"   ✅ 成功读取CSV订单清单: {len(self.order_csv_df)}个订单")
            
            # 显示列名
            print(f"   📊 CSV列名: {list(self.order_csv_df.columns)}")
            
            # 统计订单类型
            order_types = {}
            for _, row in self.order_csv_df.iterrows():
                order_no = str(row.get('订单号', '')).strip()
                if order_no.startswith('PSO'):
                    order_types['PSO'] = order_types.get('PSO', 0) + 1
                elif order_no.startswith('RSO'):
                    order_types['RSO'] = order_types.get('RSO', 0) + 1
                elif order_no.startswith('MSO'):
                    order_types['MSO'] = order_types.get('MSO', 0) + 1
                elif order_no.startswith('TSO'):
                    order_types['TSO'] = order_types.get('TSO', 0) + 1
                elif order_no.startswith('P-R'):
                    order_types['P-R'] = order_types.get('P-R', 0) + 1
            
            print(f"   📋 订单类型分布: {order_types}")
            
            # 显示前5个和后5个订单
            print("   🔍 前5个订单:")
            for i in range(min(5, len(self.order_csv_df))):
                row = self.order_csv_df.iloc[i]
                print(f"      {i+1}. {row.get('订单号', '')}: {row.get('客戶型號', '')} ({row.get('訂單數量', 0)}pcs)")
            
            return True
            
        except Exception as e:
            print(f"   ❌ CSV订单清单加载失败: {e}")
            return False
    ...
    def analyze_csv_orders_shortage(self):
        """分析CSV订单清单的欠料情况"""
        print("=== 🎯 分析CSV订单清单欠料情况 ===")
        
        if self.order_csv_df is None or self.shortage_df is None:
            print("❌ 数据未准备完成")
            return False
        ...
        print(f"开始分析{len(self.order_csv_df)}个订单...")
        
        for i, row in self.order_csv_df.iterrows():
            if (i + 1) % 50 == 0:
                print(f"   处理进度: {i + 1}/{len(self.order_csv_df)}")
            
            # 获取CSV中的订单信息
            csv_order_no = str(row.get('订单号', '')).strip()
            csv_pr_order = str(row.get('P-R订单号转换', '')).strip()
            csv_line = str(row.get('生产线', '')).strip()
            csv_factory_model = str(row.get('本廠型號', '')).strip()
            csv_customer_model = str(row.get('客戶型號', '')).strip()
            csv_quantity = pd.to_numeric(row.get('訂單數量', 0), errors='coerce')
            csv_ots_date = row.get('OTS期', '')
            csv_sequence = row.get('序号', i + 1)
            csv_excel_row = row.get('Excel行号', '')
            
            # 记录P-R订单
            if csv_order_no.startswith('P-R'):
                pr_orders.append(csv_order_no)
            
            # 1. 查找原始订单信息（获取订单金额）
            order_info = self.orders_df[self.orders_df['生产单号'] == csv_order_no]
            ...
            if not order_info.empty:
                ...
                delivery_date = order_base.get('客户交期', csv_ots_date)
                customer_order = order_base.get('客户订单号', '')
            else:
                # 如果原始订单表中没有，使用默认值
                order_amount_usd = 1000  # 默认订单金额
                order_amount_rmb = order_amount_usd * self.currency_rates['USD']
                month = ''
                source = ''
                bom_no = ''
                delivery_date = csv_ots_date
                customer_order = ''
            
            # 2. 查找欠料信息
            # 尝试多种匹配方式：订单号、P-R转换号
            shortage_records = pd.DataFrame()
            
            # 方式1: 直接匹配订单号
            shortage_records = self.shortage_df[self.shortage_df['订单编号'].astype(str).str.strip() == csv_order_no]
            
            # 方式2: 如果没找到，尝试匹配P-R转换号
            if shortage_records.empty and csv_pr_order and csv_pr_order != 'nan':
                shortage_records = self.shortage_df[self.shortage_df['订单编号'].astype(str).str.strip() == csv_pr_order]
            ...
            if not shortage_records.empty:
                ...
                for _, shortage_row in shortage_records.iterrows():
                    ...
                    record = {
                        'CSV序号': csv_sequence,
                        'Excel行号': csv_excel_row,
                        '生产线': csv_line,
                        '订单号': csv_order_no,
                        'P-R订单号转换': csv_pr_order,
                        '本厂型号': csv_factory_model,
                        '客户型号': csv_customer_model,
                        '订单数量Pcs': csv_quantity,
                        'OTS期': csv_ots_date,
        ...
        if pr_orders:
            print(f"   🔍 P-R订单列表（前10个）: {pr_orders[:10]}")
    ...
    def calculate_roi_and_priority(self):
        ...
        for idx, row in order_summary.iterrows():
            order_records = self.final_result[self.final_result['订单号'] == row['订单号']]
        ...
        # 5. 提取高优先级订单（ROI>=2.0或无需投入）
        high_priority_orders = priority_sorted[
            (priority_sorted['ROI'] >= 2.0) | 
            (priority_sorted['ROI'] >= 999999)
        ].copy()
        ...
        recommended_production = len(high_priority_orders)
        ...
        print(f"   💡 建议优先生产: {recommended_production}个 ({recommended_production/total_orders*100:.1f}%)")
        
        # 保存汇总结果
        self.order_summary = order_summary
        self.priority_sorted = priority_sorted
        self.high_priority_orders = high_priority_orders
    ...
    def generate_summary_reports(self):
        ...
        if not shortage_records.empty:
            material_summary = shortage_records.groupby(['欠料物料编号', '欠料物料名称']).agg({
                ...
                '库存RMB单价': 'first',
                '欠料金额(RMB)': 'sum',
                '订单号': lambda x: ', '.join(sorted(x.unique())[:5])  # 显示前5个相关订单
            }).reset_index()
            
            material_summary = material_summary.sort_values('欠料金额(RMB)', ascending=False)
            material_summary.rename(columns={'订单号': '相关订单(前5个)'}, inplace=True)
        else:
    ...
    def save_comprehensive_report(self):
        ...
        filename = f'594订单CSV清单欠料分析报告_{timestamp}.xlsx'
        
        try:
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                # 1. 主报表：按CSV顺序的详细分析
                self.final_result.to_excel(writer, sheet_name='按CSV顺序详细分析', index=False)
                ...
                # 4. 高优先级生产建议（ROI>=2.0）
                self.high_priority_orders.to_excel(writer, sheet_name='建议优先生产订单', index=False)
                ...
                stats_data = {
                    ...
                    '数值': [
                        len(self.order_csv_df),
                        len(self.order_summary[self.order_summary['欠料金额(RMB)'] > 0]),
                        len(self.order_summary[self.order_summary['ROI'] >= 999999]),
                        len([o for o in self.final_result['订单号'].unique() if str(o).startswith('P-R')]),
                        len(self.high_priority_orders),
            ...
            print(f"📋 包含工作表:")
            print(f"   1️⃣ 按CSV顺序详细分析 ({len(self.final_result)}条记录)")
            print(f"   2️⃣ 按订单汇总(含ROI) ({len(self.order_summary)}个订单)")
            print(f"   3️⃣ ROI优先级排序 (全部订单排序)")
            print(f"   4️⃣ 建议优先生产订单 ({len(self.high_priority_orders)}个高ROI订单)")
            print(f"   5️⃣ 按供应商汇总 (采购部门使用)")
            print(f"   6️⃣ 采购物料清单 (具体采购清单)")
            print(f"   7️⃣ 统计汇总 (管理层概览)")
    ...
    def run_comprehensive_analysis(self):
        """运行完整的综合分析"""
        print("🚀 开始594个CSV订单欠料分析")
        print("="*70)
        
        try:
            # 1. 加载CSV订单清单
            if not self.load_csv_order_list():
                return None
            ...
            # 3. 分析CSV订单欠料
            if not self.analyze_csv_orders_shortage():
                return None
            ...
            recommended_orders = len(self.high_priority_orders)
            total_shortage = self.final_result['欠料金额(RMB)'].sum()
            total_order_value = self.order_summary['订单金额(RMB)'].sum()
            
            print(f"📊 关键结果:")
            print(f"   - 分析订单总数: {len(self.order_csv_df)}个")
            print(f"   - 可立即生产: {immediate_orders}个 ({immediate_orders/len(self.order_csv_df)*100:.1f}%)")
            print(f"   - 高优先级(ROI≥5): {high_priority}个")
            print(f"   - 建议优先生产: {recommended_orders}个 ({recommended_orders/len(self.order_csv_df)*100:.1f}%)")
            ...
            for _, order in top5_shortage.iterrows():
                if order['欠料金额(RMB)'] > 0:
                    ...
                    print(f"   {order['订单号']}: {order['客户型号']} (欠料¥{order['欠料金额(RMB)']:,.2f}, ROI {roi_text})")
...
if __name__ == "__main__":
    analyzer = CSVOrderShortageAnalyzer()
